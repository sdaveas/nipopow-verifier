...
\subsection{Removing Ancestors}

If one observes the code of the verifier, can realize that the existence of
\texttt{ancestors_s} structure is redundant. The lifetime of
\texttt{ancestors_s} is limited within the last lines of each phase, and serves
the role of temporary storing the result of texttt{evaluatePredicate}, and then
it is deleted. The same functionality can be achieved if the
\texttt{ancestor_s} is saved in memory rather than in the storage of the
contract.
