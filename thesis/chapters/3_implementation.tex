...
\[ \pi_{exist}\{:lca\} \subseteq \pi_{cont}\{:lca\} \]
This way, we delegate to $E_{cont}$ the computational burden of creating
$\pi_{cont}$ in a form that can be directly compared with $\pi_{exist}$, and we
effectively avoid the demanding need of composing auxiliary structures DAG and
ancestors on-chain. The implementation of $subset$ is displayed in
listing~\ref{listing:subset}.

\lstinputlisting[style=customc, captionpos=b, label={listing:subset},
caption={Solidity implementation of subset}]{code/Subset.sol}

\input{figures/subset_usage.tex}

The gas consumption difference is displayed at
figure~\ref{figure:DAG_vs_subset}. The $subset$ solution is approximately 2.7
