\begin{algorithm}
    ...
    \If{$\pr_1$[f] $\neq$ $\pr_2[0]$}
    \Comment{Check the fork head}
        \State\Return false
    \EndIf
    \For{$b_1\ in\ \pi_1[f{+}1{:}]$}
    \Comment{Check if proofs are disjoint}
        \If{$b_2\ in\ \pi_2[1{:}]$}
            \State\Return false
        \EndIf
    \EndFor
