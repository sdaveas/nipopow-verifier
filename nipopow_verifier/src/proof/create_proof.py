...
def make_proof_filename(blocks):
    return proof_dir() + 'proof_'+str(blocks)+'.pkl'

"""
Loads a proof from a .pkl file. The file must exist.
"""
def import_proof(blocks=None, filename=None):
    if blocks==None and filename==None:
        raise NameError('You need to provide number of blocks or filename for proof')
    if filename==None:
        filename = make_proof_filename(blocks)
...
"""
Creates a chain with the given block number, constracts the corresponding proof
and stores the proof in a .pkl file
"""
def create_proof(blocks, filename=None):
...
def make_fork_proof_filename(mainblocks, fork_index, forkblocks):
    return proof_dir() + str(mainblocks) + '_-' + str(fork_index) + '_+' + str(forkblocks) + '.pkl'

"""
Returns a proof generated by a chain of the size of the given number of blocks
If the proof doesn't exist, it will be created according to the name convension
If the proof exists, it will be imported from the proof directory
"""
def fetch_proof_by_blocks(blocks):
    try:
        proof = import_proof(blocks)
    except IOError:
        filename = make_proof_filename(blocks)
        print('File', filename, 'does not exist. Creating...')
        proof = create_proof(blocks)
...
def fetch_proof_by_name(filename):
    return import_proof(blocks=None, filename=filename)

def fetch_proof(arg):
    if type(arg) == str:
        return fetch_proof_by_name(arg)
    elif type(arg) == int:
        return fetch_proof_by_blocks(arg)
    else:
