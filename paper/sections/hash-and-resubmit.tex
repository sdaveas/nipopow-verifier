...
in the NIPoPoW protocol, and we analyze the performance in comparison with the
...
availability, which consists one of our main requirements. Furthermore,
...
also one of our critical target. Lastly, ~\cite{memory-array} proposes an
...
the size of this digest is 32 bytes. To persist such a small value in
...
these structures are large. To that extend, smart contracts that exceeded the
Ethereum block gas limit and benefit sufficiently for the alleviation of
storage structures become practical. Furthermore, the pattern enables off-chain
transactions, significantly improving the performance of smart contracts.
...
node observes the function calls towards the smart contract.  Therefore, it
consists a suitable case for our pattern.

In \emph{submit} phase, a \emph{proof} is submitted. In the case of falsity, it
...
chain\footnote{Bitcoin spans $633{,}022$ blocks as on June 2020. Metrics by
