...
interest} in proofs, which leads us to the construction of an architecture
that does not require DAG, ancestors or other complementary structures. To
support this claim, we adopt the notation from~\cite{nipopows}.
...
to be "does block $\boi$ exists inside proof $\pr$?", where $\boi$ denotes
...
$\pis$ in a block is prior to $\boi$.
...
interest is \texttt{D} and its predecessors (which we do not display).
