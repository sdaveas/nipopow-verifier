...
$\pr_1$ differs from $\pr_2$, then a fork is created at the index of their last
...
\true for $\pis$. A submission in which $\boi$ is absent from $\pis$ is aimless,
...
block of interest is \texttt{D} and its predecessors (which are not displayed
in the figure).
