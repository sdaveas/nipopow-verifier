...
def test_create_disjoint_fork_proofs(init_environment):
    ...
    This fails because pt.create_fixed_fork_proof() does not create a valid disjoint proof.
    """
    pt = ProofTool()
    original_proof = submit_proof.proof
    fork_proof = contest_proof.proof
    truncated_proof, lca = pt.create_fixed_fork_proof(original_proof, fork_proof)
